...
#LOAD IN twopatch_ext_heterograz_ID_full
load('/home/agreiner/scratch/BoA_calculations/twopatch_ext_heterograz_ID_full.RData')
print(paste("The first row of twopatch_ext_unequal_complete_IDs = ", twopatch_ext_heterograz_ID_full[1,]))
...
#350 as > the max number of stable equilibria possible at any one parameter combination
basins <- data.frame(RecruitValue=c(rep(seq(0,0.99,by=0.01),each=350*99),rep(seq(0,0.99,by=0.01),each=350*99),rep(seq(0,0.99,by=0.01),each=350*99)),Grazing=c(rep(seq(0.01,0.99,by=0.01),each=350),rep(seq(0.01,0.99,by=0.01),each=350),rep(seq(0.01,0.99,by=0.01),each=350)),grazlvl=c(rep(0.1,99*100*350),rep(0.3,99*100*350),rep(0.5,99*100*350))
,EquilibriumID=seq(1,350,by=1), Size = 0, numNA = -1)
...
CalcTrajectories <- function(i,parameters,recruitvalue, g_val, glvl, ntrajectory,times,mumbytrajectories,initM,initC,initT,initM2,initC2,initT2,MumbyOpen_Elmhirst_2PatchExt){
for(i in 1:ntrajectory){
#Elmhirst parameter model 
parameters <- c(a <- 0.1, d <- 0.24, g <- g_val, r <- 0.55, gamma <- 0.77, p_c <- recruitvalue, q_c <- recruitvalue, p_m <- recruitvalue, q_m <- recruitvalue, glvl <- glvl)
...
BOA <- function(glvl, recruitvalue,g_val,twopatch_ext_IDs,basinofattractionID,basins,ntrajectory,radius,times,finaltime){
print("In BOA, glvl = ", glvl)
...
numequi <- dim(twopatch_ext_IDs[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == recruitvalue & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl & twopatch_ext_IDs$stability == "stable_node",])[1]
#coordinates of the stable equi at that recruitment and grazing value combo
M1equi <- twopatch_ext_IDs$M1[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == recruitvalue &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == recruitvalue &  twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl &  twopatch_ext_IDs$stability == "stable_node"]
print(paste("M1equi worked and is",M1equi))
M2equi <- twopatch_ext_IDs$M2[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == recruitvalue &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl &  twopatch_ext_IDs$stability == "stable_node"]
print(paste("M2equi worked and is",M2equi))
C1equi <- twopatch_ext_IDs$C1[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == recruitvalue &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl &  twopatch_ext_IDs$stability == "stable_node"]
print(paste("C1equi worked and is",C1equi))
C2equi <- twopatch_ext_IDs$C2[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == recruitvalue &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl &  twopatch_ext_IDs$stability == "stable_node"]
...
basinofattractionID$Equilibrium[basinofattractionID$InitCond ==  n] <- 	twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == recruitvalue & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl &twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == M1equi[m] & twopatch_ext_IDs$M2 == M2equi[m] & twopatch_ext_IDs$C1 == C1equi[m] & twopatch_ext_IDs$C2 == C2equi[m]]
basins$Size[basins$RecruitValue == recruitvalue & basins$grazlvl == glvl & basins$Grazing == g_val & basins$EquilibriumID == twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == recruitvalue & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl & twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == M1equi[m] & twopatch_ext_IDs$M2 == M2equi[m] & twopatch_ext_IDs$C1 == C1equi[m] & twopatch_ext_IDs$C2 == C2equi[m]]] <- 1 + basins$Size[basins$RecruitValue == recruitvalue & basins$grazlvl == glvl & basins$Grazing == g_val & basins$EquilibriumID == twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == recruitvalue & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == recruitvalue & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$g2 == glvl & twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == M1equi[m] & twopatch_ext_IDs$M2 == M2equi[m] &twopatch_ext_IDs$C1 == C1equi[m] & twopatch_ext_IDs$C2 == C2equi[m]]]}
}}
...
mumbytraj <- CalcTrajectories(i,parameters,recruitvalue, g_val, glvl, ntrajectory,times,mumbytrajectories,initM,initC,initT,initM2,initC2,initT2,MumbyOpen_Elmhirst_2PatchExt)
mumbytrajectories <- mumbytraj
print(paste("The first row of mumbytraj = ", mumbytrajectories[1,]))
save(mumbytrajectories, file = paste0("/home/agreiner/scratch/BoA_calculations/mumbytraj_heteroggrazlong/mumbytrajectories_recr",recruitvalue,"g",g_val,"_glvl",glvl,"_20000.RData"))

#code each initial condition as determined by which basin of attraction it's in
output <- BOA(glvl, recruitvalue,g_val,twopatch_ext_heterograz_ID_full,basinofattractionID,basins,ntrajectory,radius,times,finaltime)
basinofattractionID <- output[[1]]
basins <- output[[2]]
basins$Grazing <- as.numeric(as.character(basins$Grazing))
basins$RecruitValue <- as.numeric(as.character(basins$RecruitValue))
basins$grazlvl <- as.numeric(as.character(basins$grazlvl))
save(basinofattractionID, file = paste0("/home/agreiner/scratch/BoA_calculations/BoAID_heteroggrazlong/basinofattractionID_recr",recruitvalue,"g",g_val,"_glvl",glvl,"percentofcoral_20000.RData"))
save(basins, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_heteroggrazlong/basins_recr",recruitvalue,"g",g_val,"_glvl",glvl,"_20000.RData"))
basinsabr <- basins[basins$RecruitValue == recruitvalue & basins$grazlvl == glvl & basins$Grazing == g_val,]
save(basinsabr, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_heteroggrazlong_abr/basins_recr",recruitvalue,"g",g_val,"_glvl",glvl,"_20000.RData"))


#save whether a set of parameters has any NAs and how many it has
basins$numNA[basins$RecruitValue == recruitvalue & basins$grazlvl == glvl & basins$Grazing == g_val] <-length(which(is.na(basinofattractionID$Equilibrium)))
save(basins, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_heteroggrazlong/basins_recr",recruitvalue,"g",g_val,"_glvl",glvl,"_20000.RData"))
basinsabr <- basins[basins$RecruitValue == recruitvalue & basins$grazlvl == glvl & basins$Grazing == g_val,]
save(basinsabr, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_heteroggrazlong_abr/basins_recr",recruitvalue,"g",g_val,"_glvl",glvl,"_20000.RData"))
