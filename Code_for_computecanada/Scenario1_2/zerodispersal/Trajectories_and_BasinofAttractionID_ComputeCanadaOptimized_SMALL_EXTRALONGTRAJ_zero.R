...
#LOAD IN two_patch_ext_zero_ID_full 
load('/home/agreiner/scratch/BoA_calculations/twopatch_ext_zero_ID_full.RData')
print(paste("The first row of twopatch_ext_zero_ID_full = ", twopatch_ext_zero_ID_full[1,]))
...
basins <- data.frame(RecruitValue=c(rep(0,each=100*99)),
Grazing=c(rep(seq(0.01,0.99,by=0.01),each=100)),percentofcoral=c(rep(1,99*100))
...
save(mumbytrajectories, file = paste0("/home/agreiner/scratch/BoA_calculations/mumbytraj_zero/mumbytrajectories_recr",recruitvalue,"g",g_val,"_",lvl*100,"percentofcoral.RData"))

#code each initial condition as determined by which basin of attraction it's in

output <- BOA(lvl, recruitvalue,g_val,twopatch_ext_zero_ID_full,basinofattractionID,basins,ntrajectory,radius,times,finaltime)
basinofattractionID <- output[[1]]
basins <- output[[2]]
basins$Grazing <- as.numeric(as.character(basins$Grazing))
basins$RecruitValue <- as.numeric(as.character(basins$RecruitValue))
basins$percentofcoral <- as.numeric(as.character(basins$percentofcoral))
save(basinofattractionID, file = paste0("/home/agreiner/scratch/BoA_calculations/BoAID_zero/basinofattractionID_recr",recruitvalue,"g",g_val,"_",lvl*100,"percentofcoral_10000.RData"))
...
save(basins, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_zero/basins_recr",recruitvalue,"g",g_val,"_",lvl*100,"percentofcoral_10000.RData"))
basinsabr <- basins[basins$RecruitValue == recruitvalue & basins$percentofcoral == lvl & basins$Grazing == g_val,]
save(basinsabr, file = paste0("/home/agreiner/scratch/BoA_calculations/bsns_zero_abr/basins_recr",recruitvalue,"g",g_val,"_",lvl*100,"percentofcoral_10000.RData"))
