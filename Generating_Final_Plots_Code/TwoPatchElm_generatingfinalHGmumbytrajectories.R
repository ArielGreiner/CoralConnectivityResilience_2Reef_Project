...
recruitvalue = c(0.8,0.8,0.1, 0.05, 0.17, 0)
g_lvl = c(0.1,0.5,0.5, 0.5, 0.1, 0.5)
g_val = c(0.6,0.2,0.1,0.25,0.55,0.1)

for(j in 1:length(g_lvl)){
parameters <- c(a <- 0.1, d <- 0.24, g <- g_val[j], r <- 0.55, gamma <- 0.77, p_c <- recruitvalue[j], q_c <- recruitvalue[j], p_m <- recruitvalue[j], q_m <- recruitvalue[j], glvl <- g_lvl[j])
mumbytraj <- CalcTrajectories(i,parameters,recruitvalue[j], g_val[j], g_lvl[j], ntrajectory,times,mumbytrajectories,initM,initC,initT,initM2,initC2,initT2,MumbyOpen_Elmhirst_2PatchExt)
mumbytrajectories <- mumbytraj

save(mumbytrajectories, file = paste0("~/Dropbox/University of Toronto/Research Related/Chapter Drafts/Bistability Connectivity Chapter/data_for_final_phaseportraits/mumbytrajectories_recr",recruitvalue[j],"g",g_val[j],"_glvl",g_lvl[j],"_20000.RData"))
