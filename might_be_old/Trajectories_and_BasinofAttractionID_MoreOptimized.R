...
#30 as > the max number of stable equilibria possible at any one parameter combination
basins <- data.frame(RecruitValue=c(rep(seq(0.01,0.99,by=0.01),each=30*99),rep(seq(0.01,0.99,by=0.01),each=30*99),rep(seq(0.01,0.99,by=0.01),each=30*99),rep(seq(0.01,0.99,by=0.01),each=30*99),rep(seq(0.01,0.99,by=0.01),each=30*99)),
Grazing=c(rep(seq(0.01,0.99,by=0.01),each=30),rep(seq(0.01,0.99,by=0.01),each=30),rep(seq(0.01,0.99,by=0.01),each=30),rep(seq(0.01,0.99,by=0.01),each=30),rep(seq(0.01,0.99,by=0.01),each=30)),percentofcoral=c(rep(1,99*99*30),rep(0.05,99*99*30),rep(0.1,99*99*30),rep(0.25,99*99*30),rep(0.5,99*99*30))
,EquilibriumID=seq(1,30,by=1), Size = 0)
...
times <- seq(0,585, by = 0.1) #changed from 0,100 by = 0.1
...
BOAframe <- data.frame(Run = rep(seq(1,ntrajectory,by=1),each=numequi),M1equi <- twopatch_ext_IDs$M1[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) &  twopatch_ext_IDs$g == g_val &  twopatch_ext_IDs$stability == "stable_node"], M2equi <- twopatch_ext_IDs$M2[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val &  twopatch_ext_IDs$stability == "stable_node"], C1equi <- twopatch_ext_IDs$C1[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val &  twopatch_ext_IDs$stability == "stable_node"],C2equi <- twopatch_ext_IDs$C2[twopatch_ext_IDs$p_c == recruitvalue &  twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) &  twopatch_ext_IDs$q_c == recruitvalue &  twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val &  twopatch_ext_IDs$stability == "stable_node"])

for(n in 1:dim(BOAframe)[1]){
print(paste("n = ",n))
if((((BOAframe$M1equi[n] - radius) < mumbytrajectories$M1[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)]) && ((BOAframe$M1equi[n] + radius) > mumbytrajectories$M1[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)])) & (((BOAframe$C1equi[n] - radius) < mumbytrajectories$C1[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)]) && ((BOAframe$C1equi[n] + radius) > mumbytrajectories$C1[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)])) & (((BOAframe$M2equi[n] - radius) < mumbytrajectories$M2[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)]) && ((BOAframe$M2equi[n] + radius) > mumbytrajectories$M2[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)])) & (((BOAframe$C2equi[n] - radius) < mumbytrajectories$C2[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)]) && ((BOAframe$C2equi[n] + radius) > mumbytrajectories$C2[mumbytrajectories$Run == BOAframe$Run[n] & mumbytrajectories$TimeStep > (length(times) - finaltime)]))){
basinofattractionID$Equilibrium[basinofattractionID$InitCond ==  BOAframe$Run[n]] <- 	twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val &twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == BOAframe$M1equi[n] & twopatch_ext_IDs$M2 == BOAframe$M2equi[n] & twopatch_ext_IDs$C1 == BOAframe$C1equi[n] & twopatch_ext_IDs$C2 == BOAframe$C2equi[n]]
basins$Size[basins$RecruitValue == recruitvalue & basins$percentofcoral == lvl & basins$Grazing == g_val & basins$EquilibriumID == twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == BOAframe$M1equi[n] & twopatch_ext_IDs$M2 == BOAframe$M2equi[n] & twopatch_ext_IDs$C1 == BOAframe$C1equi[n] & twopatch_ext_IDs$C2 == BOAframe$C2equi[n]]] <- 1 + basins$Size[basins$RecruitValue == recruitvalue & basins$percentofcoral == lvl & basins$Grazing == g_val & basins$EquilibriumID == twopatch_ext_IDs$ID[twopatch_ext_IDs$p_c == recruitvalue & twopatch_ext_IDs$p_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$q_c == recruitvalue & twopatch_ext_IDs$q_m == round(recruitvalue*lvl,4) & twopatch_ext_IDs$g == g_val & twopatch_ext_IDs$stability == "stable_node" & twopatch_ext_IDs$M1 == BOAframe$M1equi[n] & twopatch_ext_IDs$M2 == BOAframe$M2equi[n] & twopatch_ext_IDs$C1 == BOAframe$C1equi[n] & twopatch_ext_IDs$C2 == BOAframe$C2equi[n]]]}
}
...
save(mumbytrajectories, file = paste0("mumbytrajectories_recr",recruitvalue,"g",g_val,lvl*100,"percentofcoral.RData"))
...
basinofattractionID <- output$basinofattractionID
basins <- output$basins
save(basinofattractionID, file = paste0("basinofattractionID_recr",recruitvalue,"g",g_val,lvl*100,"percentofcoral_5850.RData"))
save(basins, file = paste0("basins_updateduntil_recr",recruitvalue,"g",g_val,lvl*100,"percentofcoral_5850.RData"))
}
...
#functions within functions, any issues there?
#check whether '30' is still bigger than the number of stable equi at a particular parameter combo
#for(i in 1:range(twopatch_ext_unequal_complete_IDs$paramcombo)[2])
#if(length(twopatch_ext_unequal_complete_IDs$percentofcoral[twopatch_ext_unequal_complete_IDs$paramcombo == i & twopatch_ext_unequal_complete_IDs$stability == "stable_node"]) > 29){
#print(length(twopatch_ext_unequal_complete_IDs$percentofcoral[twopatch_ext_unequal_complete_IDs$paramcombo == i & twopatch_ext_unequal_complete_IDs$stability == "stable_node"]))
#break	
#}
# => yes it is
